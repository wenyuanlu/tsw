...
import com.maishuo.tingshuohenhaowan.databinding.ViewAgreementDialogLayoutBinding
...
class UserAgreementDialog : BaseDialogFragment {
    ...
    private var binding: ViewAgreementDialogLayoutBinding? = null
    ...
    override fun fetchRootView(): View? {
        binding = ViewAgreementDialogLayoutBinding.inflate(LayoutInflater.from(context))
        return binding?.root
    }
    ...
    private fun getClickableSpan(): SpannableString {
        ...
        }, 39, 52, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        spannableString.setSpan(NoUnderlineSpan(), 39, 52, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)

        spannableString.setSpan(object : ClickableSpan() {
            override fun onClick(widget: View) {
                WebViewActivity.to(context, PRIVACY_SERVICES_AGREEMENT_URL, "隐私政策")
            }
        }, 53, 68, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
        spannableString.setSpan(NoUnderlineSpan(), 53, 68, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
    ...
    override fun initWidgetsEvent() {
        binding?.agreementDialogCancel?.setOnClickListener {
            dismiss()
            UserAgreementConfirmDialog(appCompatActivity!!).showDialog()
        }
