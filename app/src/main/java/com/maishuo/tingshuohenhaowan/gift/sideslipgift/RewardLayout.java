...
public class RewardLayout extends LinearLayout {
    ...
    private             List<GiftIdentify>       mWaitBeanList;//等待中的队列
    private             boolean                  mIsWait           = true;//是否是等待插入 false则直接替换当前最展示时间最短的那个
    ...
    public interface GiftAdapter<T extends GiftIdentify> {
         ...
         * @return
         */
        View onUpdate (View view, T newBean, T oldBean);
    ...
    public RewardLayout (Context context) {
        super(context);
        MAX_GIFT_COUNT = MAX_COUNT_DEFAULT;
        setGiftItemRes(R.layout.gift_animation_item);
        init();
    }
     ...
     * @param sBean
     */
    private void showGift (GiftIdentify sBean, boolean formBsaket) {
        if (adapter == null) {
            return;
        }
        GiftIdentify bean = null;
        for (GiftIdentify baseGiftBean : beans) {//先比较礼物,确定是否在界面已经显示
        ...
        if (giftView == null) {
            ...
            if (getCurrentGiftCount() > MAX_GIFT_COUNT - 1) {
                if (mIsWait) {
                    //TODO:顺序等待加入操作
                    if (formBsaket) {
                        mWaitBeanList.add(mBean);
                    }
                } else {
                    //移除最后一次更新时间比较长,然后把新的插入
                ...
                addGiftViewAnim(mBean);
                if (mIsWait && !formBsaket) {//在等待集合中,展示后需要
                    mWaitBeanList.remove(0);
                }
            }
    ...
    private void removeChildGift (View view) {
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (index >= 0) {
                ...
                String giftName = bean.getTheGiftName();
                String userId   = bean.getTheUserId();
                for (Iterator<GiftIdentify> it = beans.iterator(); it.hasNext(); ) {
                    GiftIdentify value = it.next();
                    if (value.getTheGiftName().equals(giftName) && value.getTheUserId().equals(userId)) {
                        it.remove();
                    }
                }
                vg.removeView(view);
    ...
    private void takeTask () {
        ...
        try {
            GiftIdentify gift;
            boolean      formBsaket = true;
            if (mWaitBeanList != null && mWaitBeanList.size() > 0) {
                gift = mWaitBeanList.get(0);
                formBsaket = false;
            } else {
                gift = basket.takeGift();
                formBsaket = true;
            }
            if (gift != null) {
                boolean finalFormBsaket = formBsaket;
                post(new Runnable() {
                    @Override
                    public void run () {
                        LogUtils.LOGE(TAG,"showGift");
                        showGift(gift, finalFormBsaket);
                    }
                });
            }
        } catch (InterruptedException e) {
