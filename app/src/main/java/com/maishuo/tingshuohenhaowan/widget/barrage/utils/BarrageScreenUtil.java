 ...
 * <p>
 * 自动适配屏幕像素的工具类。
 * 需要先调用{@link BarrageScreenUtil#init(Context)}才能正常使用。如果屏幕旋转，
 * 那么需要再次调用{@link BarrageScreenUtil#init(Context)}以更新。
 */

@SuppressWarnings({"unused", "WeakerAccess", "SuspiciousNameCombination"})
public class BarrageScreenUtil {
    private static final String TAG          = "ScreenUtil";
    /**
     * 屏幕宽度，在调用init()之后通过{@link BarrageScreenUtil#getScreenWidth()}获取
     */
    private static       int    sScreenWidth = 1920;

    /**
     * 屏幕高度，在调用init()之后通过{@link BarrageScreenUtil#getScreenHeight()} ()}获取
     */
    private static int sScreenHeight = 1080;

    /**
     * 设计宽度。用于{@link BarrageScreenUtil#autoWidth(int)}
     */
    private static int sDesignWidth = 1080;

    /**
     * 设计高度。用于{@link BarrageScreenUtil#autoHeight(int)} (int)}
     */
